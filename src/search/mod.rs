...
/// 共享的源文件判定：统一内部 `to_lowercase()` 规范化大小写，
/// 保证本地模式与远程模式下过滤行为一致（REVIEW #67）。
///
/// 接收路径字符串，跳过常见非源代码目录并按扩展名匹配。
pub(crate) fn is_source_file_name(path: &str) -> bool {
    // 跳过常见非源代码目录（大小写不敏感，避免 `Node_Modules/` 等被纳入）
...
fn is_source_file(path: &Path) -> bool {
    is_source_file_name(&path.to_string_lossy())
}
